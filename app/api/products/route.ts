...
export async function GET() { //export here doesnt work like with normal functions; because its in a route.ts file, next knows its to be used to handle a request
  ...
  const response = await fetch("https://bfshop.benfosterdev.com/api/products"); //await pauses the funciton until the asynchronous operation finishes. awaits until dummy server responds
